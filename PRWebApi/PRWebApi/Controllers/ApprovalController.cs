...
namespace PRWebApi.Controllers
{
    ...
    public class ApprovalController : ControllerBase
    {
        ...
        UnitOfWork _uow;
        public ApprovalController(IConfiguration config, UnitOfWork uow)
        {
            _config = config;
            _uow = uow;
        ...
        public IActionResult Get()
        {
            try
            {
                IEnumerable<Approval> list = _uow.Query<Approval>();
                return Ok(list);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                Approval obj = await _uow.GetObjectByKeyAsync<Approval>(id);
                if (obj == null)
                {
                    NotFound();
                }
        ...
        public async Task<IActionResult> Post([FromBody]JObject values)
        {
            try
            {
                ...
                //_uow.CommitChanges();
                Approval obj = JsonPopulateObjectHelper.PopulateObject<Approval>(values.ToString(), _uow);
        ...
        public async Task<IActionResult> Put(int id, [FromBody]JObject value)
        {
            try
            {
                ...
                ////{
                ////    customer.Department = _uow.GetObjectByKey<Approval>(token["Oid"].Value<int>());
                ////}
